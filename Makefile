...
# Docker backend Go.
docker_be_build:
	docker build . -t shopping-cart-backend:v1.0.0
docker_be_start: docker_net_start
	docker run --network=shopping-cart --name shopping-cart-backend -p 4000:4000 -d shopping-cart-backend:v1.0.0
docker_be_stop:
...
docker_db_build:
	docker build -f ./db/Dockerfile ./db/ -t shopping-cart-postgres:v1.0.0
docker_db_start: docker_net_start
	docker run --hostname shopping-cart-postgres --network=shopping-cart --restart unless-stopped --name shopping-cart-postgres -p 5432:5432 -d shopping-cart-postgres:v1.0.0
	# Use this command if you want to save the db data.
	# docker run --hostname shopping-cart-postgres --network=shopping-cart --restart unless-stopped --name shopping-cart-postgres -p 5432:5432 -v ./db/data/.postgres/:/var/lib/postgresql/data -d shopping-cart-postgres:v1.0.0
docker_db_stop:
